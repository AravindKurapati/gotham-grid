...
**Date:** 2026-04-13  
**Status:** Approved (updated with city expansion + UI rules)

---

## Overview

GOTHAM GRID is a retro-aesthetic dashboard that aggregates vibe-coded projects from 25 global cities. It is the "Bloomberg Terminal of vibe-coding culture." Built with Next.js 16 (App Router) + React 19 + TypeScript + Tailwind v4 + Anthropic SDK.
...
- **No em dashes or long dashes** anywhere in rendered UI. Use double-hyphen `--` or spaces as separators instead. This applies to all components, data display, error messages, and the ticker.
...
- **No hand-crafted placeholder data** -- all JSON files are generated by running the scanner

### Cities (25 total)
Original 10: nyc, london, tokyo, berlin, paris, sf, seoul, mumbai, lagos, sao_paulo  
New 15: hyderabad (HYD), bangalore (BLR), delhi (DEL), chennai (MAA), dubai (DXB), singapore (SIN), sydney (SYD), toronto (YYZ), amsterdam (AMS), stockholm (ARN), tel_aviv (TLV), budapest (BUD), brussels (BRU), athens (ATH), los_angeles (LAX)

CityKey type expands to cover all 25. CitySelector shows top 8 visible + overflow menu for the rest.
...
├── data/                           # Scanner-generated JSON (25 city files)
│   └── {all 25 cities}.json        # Generated by running: npx tsx scripts/scan.ts
├── scripts/
